...
<body>
    ...
    <div class="panel panel-default panel-tasks">
  		<div class="panel-body container panel-center">
    		...
    		<div class="btn-group">
    			<a class="btn btn-default" href="javascript:formCommandSubmit('pause');" title="Pause"><span class="glyphicon glyphicon-pause"></span></a>
    			<a class="btn btn-default" href="javascript:formCommandSubmit('resume');" title="Resume"><span class="glyphicon glyphicon-play"></span></a>
    		</div>
    		<div class="btn-group">
    			...
    			<a class="btn btn-danger" href="javascript:formCommandSubmit('cancel');" title="Remove"><span class="glyphicon glyphicon-remove-circle"></span></a>
					...
					foreach ($downloads as $file) {
						...
						if ( $filter_status_result and $filter_cat_result) {
							print "<tr>";
								...
								// Size
								echo "<td style='font-size:12px;'>", CastToXBytes($file->size_done), "&nbsp;(",
									((float)$file->size_done*100)/((float)$file->size), "%)</td>";
								// Speed
								echo "<td style='font-size:12px;'>", ($file->speed > 0) ? (CastToXBytes($file->speed) . "/s") : "-", "</td>";
								// Progress
			...
			<table class="table">
				...
				<?php
					...
					foreach ($uploads as $file) {
						echo "<tr>";										
						// Name
						echo "<td style='font-size:12px;'><b>", $file->short_name, "</b></td>";
						// User name
						echo "<td style='font-size:12px;'>", $file->user_name, "</td>";			
						// Upload dimension
						echo "<td style='font-size:12px;'>", CastToXBytes($file->xfer_up), "</td>";
						// Download dimension
						echo "<td style='font-size:12px;'>", CastToXBytes($file->xfer_down), "</td>";
						// Speed
						echo "<td style='font-size:12px;'>", ($file->xfer_speed > 0) ? (CastToXBytes($file->xfer_speed) . "/s") : "-", "</td>";
					}
				?>
      </table>
